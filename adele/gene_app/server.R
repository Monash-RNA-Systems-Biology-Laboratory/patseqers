...
shinyServer(function(input, output) {
    #Removed the select experiment input as a UI element rendered in server.R and moved it to the ui.R
    ...
    go.attribute <- reactive({
        ...
        } else if(input$org.type == "mmusculus_gene_ensembl") {
            attr <- "ensembl_gene_id"
        } else {
#             if(input$org.type == "celegans_gene_ensembl")
    ...
    base.df <- reactive({
        withProgress(message = "Generating dataframe with selected inputs", 
                     value = 0, {
                         base.df <- initial.df()
                         info.df <- info.df()
...
the clicked mouse position, arranged in order of nearest distance to furthest."))
    })
    
    #Returns the closest rows to the mouse hover
    output$info_plot <- renderPrint({
        
        nearPoints(base.df(), input$plot_click, xvar = "x.mean", yvar = "y.mean", threshold = 10, 
                   maxpoints = 5)
    })
